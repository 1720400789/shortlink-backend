...
CREATE TABLE `t_link` (
                          ...
                          `origin_uri` varchar(1024) DEFAULT NULL COMMENT '原始链接',
                          `click_num` int(11) DEFAULT 0 COMMENT '点击量',
                          `gid` varchar(32) DEFAULT NULL COMMENT '分组标识',
                          `favicon` varchar(256) CHARACTER SET utf8mb4 COLLATE utf8mb3_croatian_ci DEFAULT NULL COMMENT '网站图标',
                          `enable_status` tinyint(1) DEFAULT NULL COMMENT '启用标识 0：未启用 1：已启用',
                          `created_type` tinyint(1) DEFAULT NULL COMMENT '创建类型 0：控制台创建 1：接口创建',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT 'full_short_url 与 gid 的路由表';
...
CREATE TABLE `t_link_access_stats` (
                              ...
                              `full_short_url` varchar(128) DEFAULT NULL COMMENT '完整短链接',
                              `gid` varchar(32) DEFAULT NULL COMMENT '分组标识',
                              `date` date DEFAULT NULL COMMENT '访问的日期',
...
VALUES('nurl.ink/W5M7a', 'siCwZo', '2023-11-04', 0, 0, 0, 0, 0, NOW(), NOW(), 0) ON DUPLICATE KEY UPDATE pv = pv + 1, uv = uv + 2, uip = uip + 3;
